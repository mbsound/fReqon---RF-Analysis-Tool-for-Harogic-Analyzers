...
#define APIRETVAL_ERROR_NotSupported						-97			// The device does not support calling this function.
...
/* Device function: bits [31:0] represent the device operating mode */
...
/* Device functionality: bits [63:32] represent additional sub-functions */
...
/* Dev API: Get the list of extraction multiples for the device */
HTRA_API int Device_GetDecimateFactorList(void** Device, uint32_t* DecimateFactorList, uint32_t MaxCount,uint32_t* ActualCount);

/* dev interface: get device board memory capacity (in bytes) */
HTRA_API int Device_GetOnboardMemorySize(void** Device, uint64_t* MemorySize);

/* dev interface, get device bus bandwidth: unit is Bytes/s */
HTRA_API int Device_GetBusBandwidth(void** Device, uint64_t* Bandwidth);

/* Dev interface: Get the list of functions supported by the device (SupportedFunctions). Each bit indicates whether the corresponding function is supported; refer to DEVICE_FUNCTION_xxx for specific bitmasks. */
