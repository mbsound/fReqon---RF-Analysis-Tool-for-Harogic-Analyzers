...
class NetworkDeviceInfo_TypeDef(Structure):
    _fields_ = [("DeviceUID", c_uint64),        #Equipment serial number
                ...
                ("MFWVersion", c_uint32),       #MCU Firmware version
                ("FFWVersion", c_uint32),       #FPGA Firmware version
                ("IPAddress", c_uint8 * 4),     #IP Address
...
class SWP_Profile_TypeDef(Structure):
    _fields_ = [("StartFreq_Hz",c_double),                                  #start frequency
                ...
                ("EnableIFAGC",c_uint8),                                    #IF AGC control: 0 = disabled, 1 = enabled
...
class MeasAuxInfo_TypeDef(Structure):
    _fields_ = [("MaxIndex",c_uint32),                  #Index of the maximum power value in the current packet
                ...
                ("Altitude",c_float),                   #Altitude (meters)
                ("SATHealth",c_float),                  #GNSS satellite health (SNR)
                ("IFAGCGain",c_double),                 #IF AGC gain in dB
                ("RefClkFreqOffset",c_double),          #Reference clock frequency offset in ppm
                ("nsSinceEpoch",c_uint64)]              #Timestamp in nanoseconds since epoch
...
# Analog Signal Generator: configure working status
...
# IF AGC Controls
dll.Device_SetIFOutput.argtypes = [POINTER(c_void_p), c_uint8]
dll.Device_SetIFOutput.restype = c_int

dll.Device_SetIFAGCTarget.argtypes = [POINTER(c_void_p), POINTER(c_double)]
dll.Device_SetIFAGCTarget.restype = c_int

dll.Device_SetIFAGCPeriod.argtypes = [POINTER(c_void_p), POINTER(c_double)]
dll.Device_SetIFAGCPeriod.restype = c_int

# Network Device Discovery
if hasattr(dll, 'Device_GetNetworkDeviceList'):
    dll.Device_GetNetworkDeviceList.argtypes = [
        POINTER(c_uint8),
        POINTER(NetworkDeviceInfo_TypeDef * 64),
        POINTER(c_uint8 * 4),
        POINTER(c_uint8 * 4)
    ]
    dll.Device_GetNetworkDeviceList.restype = c_int

# Hardware Function Bitmasks (New in Linux_API-SAN-828)
DEVICE_FUNCTION_SWP           = (1 << 0)
DEVICE_FUNCTION_IQS           = (1 << 1)
DEVICE_FUNCTION_RTA           = (1 << 2)
DEVICE_FUNCTION_DET           = (1 << 3)
DEVICE_FUNCTION_PNM           = (1 << 4)
DEVICE_FUNCTION_MSCAN         = (1 << 5)
DEVICE_FUNCTION_EMC           = (1 << 6)
DEVICE_FUNCTION_INTERNAL_GNSS = (1 << 32)
DEVICE_FUNCTION_POWER_MONITOR = (1 << 33)

# New Error Codes
APIRETVAL_ERROR_NotSupported     = -97
APIRETVAL_ERROR_DemodLicExpired  = -75

# Hardware Diagnostic and Introspection APIs
if hasattr(dll, 'Device_GetSupportedFunctions'):
    dll.Device_GetSupportedFunctions.argtypes = [POINTER(c_void_p), POINTER(c_uint64)]
    dll.Device_GetSupportedFunctions.restype = c_int

if hasattr(dll, 'Device_GetBusBandwidth'):
    dll.Device_GetBusBandwidth.argtypes = [POINTER(c_void_p), POINTER(c_uint64)]
    dll.Device_GetBusBandwidth.restype = c_int

if hasattr(dll, 'Device_GetOnboardMemorySize'):
    dll.Device_GetOnboardMemorySize.argtypes = [POINTER(c_void_p), POINTER(c_uint64)]
    dll.Device_GetOnboardMemorySize.restype = c_int

if hasattr(dll, 'Device_GetDecimateFactorList'):
...
if hasattr(dll, 'Device_GPIOBandSwitch'):
    dll.Device_GPIOBandSwitch.argtypes = [
        POINTER(c_void_p), c_uint16, POINTER(c_double), POINTER(c_double),
        POINTER(c_uint16), POINTER(c_uint16), POINTER(c_uint32)
    ]
    dll.Device_GPIOBandSwitch.restype = c_int

# Analog Audio Demodulation APIs
if hasattr(dll, 'ASD_Open'):
    dll.ASD_Open.argtypes = [POINTER(c_void_p)]
    dll.ASD_Open.restype = None

if hasattr(dll, 'ASD_Close'):
    dll.ASD_Close.argtypes = [POINTER(c_void_p)]
    dll.ASD_Close.restype = None

if hasattr(dll, 'ASD_Demodulate_FM'):
    dll.ASD_Demodulate_FM.argtypes = [POINTER(c_void_p), POINTER(IQStream_TypeDef), c_bool, POINTER(c_float), POINTER(c_double)]
    dll.ASD_Demodulate_FM.restype = c_int
